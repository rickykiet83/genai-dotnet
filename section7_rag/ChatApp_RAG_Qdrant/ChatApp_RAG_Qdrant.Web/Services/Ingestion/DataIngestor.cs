...
namespace ChatApp_RAG_Qdrant.Web.Services.Ingestion;
    ...
    IEmbeddingGenerator<string, Embedding<float>> embeddingGenerator)
{
    ...
    {
        using var writer = new VectorStoreWriter<string>(vectorStore, dimensionCount: IngestedChunk.VectorDimensions,
            new()
            {
        ...
        {
            logger.LogInformation("Completed processing '{id}'. Succeeded: '{succeeded}'.", result.DocumentId,
                result.Succeeded);
        }
    }
}